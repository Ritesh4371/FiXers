...
class BangaloreWumpusWorld:
    ...
    def find_path_astar(self):
        ...
        Finds optimal path from current agent position to goal
        Movement: Only up, down, left, right (NO diagonals)
        Handles obstacles: Pits (avoided), Traffic lights (higher cost), Cows (moderate cost)
        ...
        def get_cell_cost(x, y):
            """Get cost of moving to cell at (x, y)"""
            ...
            if cell_type == 'pit':
                ...
                return 10  # Moderate cost for cows
            else:
        ...
        # Track nodes in open_set for faster lookup
        open_set_hash = {start}
